...
impl VL6180X {
    ...
    pub async fn new<T: I2c>(i2c: &mut T, addr: u8) -> Result<Self, T::Error> {
        let mut s = Self { addr, offset: 0 };
    ...
    pub async fn range_status<T: I2c>(
        &mut self,
        i2c: &mut T,
    ) -> Result<RangeStatusError, T::Error> {
        Ok(RangeStatusError::from_u8(
            self.read_8(i2c, REG_RESULT_RANGE_STATUS).await?,
        ))
    }
